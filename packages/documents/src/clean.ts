...
const search = async (emailClean: string) => {
  ...
  if (activists === null) {
    ...
    // let id: any, feCount: any, dCount: any, apCount: any;

    const id = activists.get('id');
    const feCount = formEntries.count;
    const dCount = donations.count;
    const apCount = activistPressures.count;
...
const update = async (activistClean: any) => {
  ...
  console.log("Activist -", metadata, results.length);
  ...
  if (activistClean.feCount > 0) {
    ...
    console.log("Form_entries -", metadataF, resultsF.length);
  }
  ...
  if (activistClean.dCount > 0) {
    ...
    console.log("Donations -", metadataD, resultsD.length);
  }
